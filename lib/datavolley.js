...
function dateIsInTheNextWeek(startDate, endDate) {
    const now = new Date().toISOString()
      ...
      .replace(/-/g, '')
    const date = new Date();
    const oneWeekFromNow = date.setDate(date.getDate() + 7).toISOString()
      .slice(0, 10)
      .replace(/-/g, '')
    startDate = parseInt(
        startDate
    ...
    getCurrentMatches() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            self
                ...
                .then((seasonId) => {
                    console.log('Season id', seasonId);
                    return self._getSeasonCompetitions(seasonId, currentDate)
                })
                .then((competitions) =>
            ...
            request(
                ...
                function(error, response, body) {
                    ...
                    if (!error && response.statusCode == 200) {
                        ...
                        console.log("Error is", error, response.statusCode, body);
                    }
                },
                function(er1, er2) {
                    console.log(er1, er2);
