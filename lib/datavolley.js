...
//                console.log(data);
                ...
                if (!updateOnly) {
                    ...
                    gameData.homeTeam.statistics.total.opponentErrors = awayStats.errors;

                    gameData.awayTeam.statistics.total = awayStats;
                    gameData.awayTeam.statistics.total.opponentErrors = homeStats.errors;
                    ...
                    iterate.each(new Array(gameData.currentSet), function (value, key, done) {
                        var setNum = key+1;

                        var cacheKey = self.fedCode + '_' + self.matchId + '_' + setNum;
                        ...
                        if (value) {
                            ...
                            self.service.getJson('MatchLiveStats', '/Set/' + setNum).then ( function (data) {
                                console.log(' - Got stats from match live stats');
                                // Allow 30 sek of cache for current set:
                                var cacheTime = 30;
                                if (setNum < gameData.currentSet) {
                                    cacheTime = 1800;
                                }
