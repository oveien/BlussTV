...
//                console.log(data);
    ...
    getRoster () {
        ...
        return new Promise(function (resolve, reject) {
            self.service.getJson('Roster').then((data) => {
                var htPlayers = [];
                for (var i in data.RH) {
                    var player = {
                        number: parseInt(data.RH[i].N),
                        name: data.RH[i].NM + ' ' + data.RH[i].SR,
                        height: '',
                        position: '',
                        birthyear: '',
                        reach: '',
                    ...
                    self.downloadPlayerImage('home', player.id);
                }

                var atPlayers = [];
                for (var i in data.RG) {
                    var player = {
                        number: parseInt(data.RG[i].N),
                        name: data.RG[i].NM + ' ' + data.RG[i].SR,
                        height: '',
                        position: '',
                        birthyear: '',
                        reach: '',
                    ...
                    self.downloadPlayerImage('away', player.id);
                }
    ...
    downloadPlayerImage(team, playerId) {
        var teamId = -1;
        var teamName = "";
        if (team == 'home') {
        ...
        if (!fs.existsSync(fileName)) {
            ...
            downloadFile(
                'https://dataprojectimagestorage.blob.core.windows.net:443/nvbf/TeamPlayer/TeamPlayer_'+teamId+'_'+playerId+'.jpg',
                fileName,
                function () {
                    // Callback
                }
